...
// === КОНФИГУРАЦИЯ ===
// Берем токены из Environment Variables на Render
...
try {
  ...
  console.error('❌ ОШИБКА ИНИЦИАЛИЗАЦИИ FIREBASE (проверь файл firebase-key.json):', firebaseInitError);
}
...
bot.start(async (ctx) => {
    ...
    try {
        // Здесь происходит запрос к Firestore
        ...
        // 🔥 ВЫВОДИМ ПОЛНУЮ ТЕХНИЧЕСКУЮ ОШИБКУ В КОНСОЛЬ VS CODE
        console.error('\n🛑 [FIREBASE CRITICAL ERROR] Ошибка при вызове /start внутри Firestore:');
        console.error(e); 
        console.error('-----------------------------------------------------\n');
...
bot.on('text', async (ctx) => {
    ...
    if (session.step === 'birth') {
        ...
        try {
            ...
            console.error('\n🛑 [FIREBASE CRITICAL ERROR] Ошибка сохранения данных в Firestore:');
            console.error(e);
            console.error('-----------------------------------------------------\n');
...
bot.action('menu_profile', async (ctx) => {
    ...
    try {
        ...
        console.error('Ошибка в menu_profile:', e);
    }
...
bot.action('menu_download', async (ctx) => {
    const text = `📱 **Як додати наш сервіс на головний екран як додаток:**\n\n` +
                 ...
                 `Після этого іконка з'явиться на робочому столі як звичайний додаток!`;
...
bot.action('menu_token', async (ctx) => {
    ...
    try {
        ...
        console.error('Ошибка в menu_token:', e);
    }
    try { await ctx.answerCbQuery(); } catch (e) { console.log("Канапка застаріла"); }
});
...
bot.action('menu_referral', async (ctx) => {
    ...
    try {
        ...
        console.error('Ошибка в menu_referral:', e);
    }
    try { await ctx.answerCbQuery(); } catch (e) { console.log("Канапка застаріла"); }
});
...
// Разрешаем CORS, чтобы GitHub Pages мог общаться с твоим Express-сервером
...
app.get('/user/:token', async (req, res) => {
    ...
    try {
        ...
        // ВАЖНО: Если бот работает локально, адрес http://localhost:3000
        // Когда деплоишь на Render, замени этот урл на адрес своего сервера Render!
        let photoUrl = "./photo.jpeg"; 
        if (userRawData.photo_file_id) {
            photoUrl = `https://diia-backend-zoms.onrender.com/user-photo/${userRawData.photo_file_id}`;
        }
        ...
        res.json({
            ...
            photo: photoUrl, // Теперь отдаем безопасную ссылку на наш прокси
...
// Новый эндпоинт-прокси для картинок
app.get('/user-photo/:fileId', async (req, res) => {
    try {
        const fileId = req.params.fileId;
        const fileLink = await bot.telegram.getFileLink(fileId);
        
        // Скачиваем картинку самим сервером и стримим её в React
        ...
        console.error("Помилка проксі фото:", err.message);
        res.status(404).send("Photo not found");
    }
});


// === СБРОС ВЕБХУКА И ЗАПУСК ПОД КЛЮЧ ===
app.listen(PORT, () => {
...
bot.telegram.deleteWebhook()
    ...
    .catch((err) => {
        console.error('Ошибка старта Telegraf:', err);
    });
